        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Sensor Failure Compositionality: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robust multimodal robot learning. The submission-hardened claim is intentionally narrow: Study when failures of sensors compose predictably versus catastrophically. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Sensor failure compositionality should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.413 $\pm$ 0.021 & 0.449 $\pm$ 0.016 & 0.335 $\pm$ 0.012 \\
Data-augmented WAM & 0.489 $\pm$ 0.013 & 0.526 $\pm$ 0.024 & 0.426 $\pm$ 0.017 \\
Scalar uncertainty & 0.568 $\pm$ 0.013 & 0.586 $\pm$ 0.016 & 0.505 $\pm$ 0.010 \\
Ensemble disagreement & 0.598 $\pm$ 0.013 & 0.623 $\pm$ 0.017 & 0.539 $\pm$ 0.024 \\
Conformal risk filter & 0.607 $\pm$ 0.008 & 0.626 $\pm$ 0.017 & 0.559 $\pm$ 0.010 \\
Proposed branch mechanism & 0.706 $\pm$ 0.011 & 0.706 $\pm$ 0.012 & 0.659 $\pm$ 0.014 \\
Oracle branch upper bound & 0.776 $\pm$ 0.013 & 0.770 $\pm$ 0.016 & 0.742 $\pm$ 0.014 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.652 $\pm$ 0.011 & all components \\
Minus branch atlas & 0.546 $\pm$ 0.014 & removes explicit unrealized futures \\
Minus tail risk & 0.578 $\pm$ 0.016 & uses mean predicted value only \\
Minus diagnostic probe & 0.596 $\pm$ 0.018 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.608 $\pm$ 0.008 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/103.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Sensor Failure Compositionality is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
