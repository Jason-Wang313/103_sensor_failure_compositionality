        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Sensor Failure Compositionality: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robust multimodal robot learning. The motivating bet is: Study when failures of sensors compose predictably versus catastrophically.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For robust multimodal robot learning, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
\midrule
observed\_only\_wam & nominal & 0.578 & 0.422 \\
observed\_only\_wam & hidden\_mode\_shift & 0.177 & 0.823 \\
augmentation\_as\_data\_wam & nominal & 0.588 & 0.412 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.210 & 0.790 \\
scalar\_uncertainty\_wam & nominal & 0.660 & 0.340 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.318 & 0.682 \\
proposed\_branch\_mechanism & nominal & 0.790 & 0.210 \\
proposed\_branch\_mechanism & hidden\_mode\_shift & 0.560 & 0.440 \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        \section{Conclusion}
        Sensor Failure Compositionality argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
